...
function smtpCommand(socket: tls.TLSSocket, command: string, expected: number | number[] = 250): Promise<string> {
...
async function sendEmail(to: string, subject: string, text: string) {
  ...
  if (!user || !pass || !from) throw new Error("SMTP is not configured");
  ...
  const body = [
    `From: ArveX Hosting <${from}>`, `To: ${to}`, `Subject: ${subject}`, "MIME-Version: 1.0", "Content-Type: text/plain; charset=UTF-8", "", text, ".",
  ].join("\r\n");
...
async function issueCode(userId: string, email: string, purpose: "verify_email" | "login" | "reset_password") {
  ...
  const label = purpose === "verify_email" ? "verify your ArveX account" : purpose === "reset_password" ? "reset your ArveX password" : "sign in to your ArveX account";
  await sendEmail(email, `Your ArveX verification code: ${code}`, `Your ArveX code is ${code}.\n\nUse this code to ${label}. It expires in 10 minutes.\n\nIf you did not request this, you can safely ignore this email.`);
}

async function verifyCode(userId: string, purpose: string, code: string) {
  ...
  if (!crypto.timingSafeEqual(Buffer.from(hashCode(code), "utf8"), Buffer.from(row.codeHash, "utf8"))) {
    await db.update(verificationCodesTable).set({ attempts: String(attempts + 1) }).where(eq(verificationCodesTable.id, row.id));
    return false;
  }
...
async function bootstrapAdmin() {
  ...
  if (!existing) {
    await db.insert(usersTable).values({ id: crypto.randomUUID(), email, passwordHash: hashPassword(password), emailVerified: true, role: "admin", firstName: "ArveX", lastName: "Admin" });
  } else if (existing.role !== "admin") {
    await db.update(usersTable).set({ role: "admin", passwordHash: hashPassword(password), emailVerified: true }).where(eq(usersTable.id, existing.id));
  }
}

router.use(async (_req, _res, next) => { try { await ensureSchema(); await bootstrapAdmin(); next(); } catch (error) { next(error); } });
...
router.post("/auth/signup", async (req, res) => {
  ...
  const firstName = String(req.body?.firstName ?? "").trim().slice(0, 80) || null;
  const lastName = String(req.body?.lastName ?? "").trim().slice(0, 80) || null;
  ...
  try { await issueCode(user.id, email, "verify_email"); } catch (error) { await db.delete(usersTable).where(eq(usersTable.id, user.id)); return res.status(503).json({ error: error instanceof Error ? error.message : "Email delivery is not configured." }); }
...
router.post("/auth/verify-email", async (req, res) => {
  ...
  if (!/^\d{6}$/.test(code) || !(await verifyCode(user.id, "verify_email", code))) return res.status(400).json({ error: "Invalid or expired verification code." });
  const [updated] = await db.update(usersTable).set({ emailVerified: true }).where(eq(usersTable.id, user.id)).returning();
  const sid = await createSession(publicUser(updated));
  res.cookie(SESSION_COOKIE, sid, { httpOnly: true, secure: process.env.NODE_ENV === "production", sameSite: "lax", path: "/", maxAge: SESSION_TTL });
...
router.post("/auth/login", async (req, res) => {
  ...
  try { await issueCode(user.id, email, "login"); } catch (error) { return res.status(503).json({ error: error instanceof Error ? error.message : "Email delivery is not configured." }); }
...
router.post("/auth/verify-login", async (req, res) => {
  ...
  const sid = await createSession(publicUser(user));
  res.cookie(SESSION_COOKIE, sid, { httpOnly: true, secure: process.env.NODE_ENV === "production", sameSite: "lax", path: "/", maxAge: SESSION_TTL });
  return res.json({ ok: true, user: publicUser(user) });
});

router.post("/auth/resend", async (req, res) => {
  const email = normalizeEmail(req.body?.email); const purpose = req.body?.purpose === "login" ? "login" : "verify_email";
  const [user] = await db.select().from(usersTable).where(eq(usersTable.email, email)).limit(1);
  if (!user) return res.json({ ok: true });
  try { await issueCode(user.id, email, purpose); } catch (error) { return res.status(429).json({ error: error instanceof Error ? error.message : "Please try again later." }); }
  return res.json({ ok: true });
});

router.post("/auth/forgot-password", async (req, res) => {
  const email = normalizeEmail(req.body?.email); const [user] = await db.select().from(usersTable).where(eq(usersTable.email, email)).limit(1);
  if (user) { try { await issueCode(user.id, email, "reset_password"); } catch {} }
  return res.json({ ok: true, message: "If that email exists, a reset code has been sent." });
});

router.post("/auth/reset-password", async (req, res) => {
  const email = normalizeEmail(req.body?.email); const code = String(req.body?.code ?? ""); const password = String(req.body?.password ?? "");
  if (!validPassword(password)) return res.status(400).json({ error: "Password must be 8–128 characters." });
  const [user] = await db.select().from(usersTable).where(eq(usersTable.email, email)).limit(1);
  if (!user || !(await verifyCode(user.id, "reset_password", code))) return res.status(400).json({ error: "Invalid or expired reset code." });
  await db.update(usersTable).set({ passwordHash: hashPassword(password), emailVerified: true }).where(eq(usersTable.id, user.id));
  return res.json({ ok: true });
});
